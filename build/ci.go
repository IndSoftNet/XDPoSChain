...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 13 {
			...
			log.Println("XDC requires at least Go version 1.13 and cannot")
...
func doArchive(cmdline []string) {
	...
	for _, archive := range []string{XDC, alltools} {
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser   = flag.String("sftp-user", "", `Username for SFTP upload (usually "XDC-ci")`)
...
func doAndroidArchive(cmdline []string) {
	...
	if *local {
		...
		os.Rename("XDC-sources.jar", filepath.Join(GOBIN, "XDC-sources.jar"))
		return
	}
